...
<v-container fluid class="message ">
    <div class="text-center">
        <v-alert v-if="successMessage" v-model="successMessage" type="success" dismissible dense text class="my-4 custom-alert" @input="reloadPage">
            <div class="text-h6">{{ successMessage }} </div>
        </v-alert>
        <v-alert v-if="errorMessage" v-model="errorMessage" type="error" dismissible dense text class="my-4 custom-alert" @input="reloadPage">
    ...
    <v-dialog v-model="dialogAsset" persistent max-width="700px">
        <v-card class="rounded-lg">
            ...
            <v-card-text class="my-5">
                <v-form ref="assetEdit" @submit.prevent="assignAsset">
                    <v-row>
                        <v-col cols="12" sm="12" md="12" lg="12">
                            <v-text-field type="text" label="Name" color="blue " disabled class="rounded-lg" outlined dense  v-model="assetEdit.computer_type"></v-text-field>
                        </v-col>
                    </v-row>
                    <v-row>
                        <v-col cols="12" sm="12" md="12" lg="12">
                            <v-select label="--Select Staff--" item-text="lname" class="rounded-lg" outlined dense item-value="id" color="blue lighten-1" v-model="assetEdit.staff_id" :items="staffs" :rules="staffRules"></v-select>
                        </v-col>
                    </v-row>
                </v-form>
            </v-card-text>
    ...
    <v-dialog v-model="dialogAssetUnassign" persistent max-width="700px">
        <v-card class="rounded-lg">
            <v-card-title class="blue  white--text">
                <span class="text-h5">UnAssign Asset</span>
                <v-spacer></v-spacer>
                <v-icon color="white" @click="dialogAssetUnassign = false">mdi-close</v-icon>
            </v-card-title>
            <v-card-text class="my-5">
                <v-form ref="unassign" @submit.prevent="unassignAsset">
                    <v-row class="mt-5">
                        <v-col cols="12" sm="6" md="6">
                            <v-text-field type="text" outlined dense class="rounded-lg " label="Name" v-model="electronicUn.computer_type" disabled></v-text-field>
                        </v-col>
                        <v-col cols="12" sm="6" md="6">
                            <v-select label="--Select Condition--" class="rounded-lg" outlined dense v-model="unassign.condition" :items="statusOptions " :rules="conditionRules">
                            </v-select>
                        </v-col>
                    </v-row>
                    <v-col cols="12" sm="12" md="12" class="">
                        <v-textarea label="Enter the Reason" v-model="unassign.reason" placeholder="Reason" outlined rows="3" class="rounded-lg" :rules="reasonRules" />
                    </v-col>
                    <v-row>
            ...
            <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn class="blue  white--text btn rounded-lg" text elevation="24" type="submit" @click="unassignAsset">
                    Unassign
                </v-btn>
            </v-card-actions>
        </v-card>
    ...
    <v-data-table :headers="headers" :items="filteredAssets" :page.sync="page" :items-per-page="itemsPerPage" hide-default-footer :search="search" class="elevation-0 " @page-count="pageCount = $event">
                ...
                <v-menu offset-y>
                    ...
                    <v-list>
                        ...
                        <template v-if="item.asset.status==0">
                            <v-list-item class="custom-list-item">
                                <v-list-item-title>
                                    <v-btn text small @click="editAsset(item)"  class="btn" color="blue lighten-1">
                                        <v-icon>mdi-lock</v-icon> Assign
                                    </v-btn>
                                </v-list-item-title>
                            </v-list-item>
                        </template>
                        <template v-if="item.asset.status==1">
                            <v-list-item class="custom-list-item">
                                <v-list-item-title>
                                    <v-btn text small @click="unassignAssetmodal(item)"  class="btn" color="blue lighten-1">
                                        <v-icon>mdi-lock-open</v-icon> Unassign
                                    </v-btn>
                                </v-list-item-title>
                            </v-list-item>
                        </template>
        ...
        <template v-slot:[`item.fullName`]="{ item }">
            ...
            <span v-if="item.asset.staff">
                {{ getFullName(item.asset.staff) }}
            </span>
...
export default {
    ...
    computed: {
        reasonRules() {
            return [v => !!v || 'Reason is required'];
        },
        conditionRules() {
            return [v => !!v || 'Select condition please'];
        },
        staffRules() {
            return [v => !!v || 'Staff is required'];
        },
        startIndex() {
    ...
    methods: {
        getFullName(staff) {
            ...
            // const middleName = mname ? ` ${mname}` : ''; // Include middle name if present
            return `${fname} ${lname}`;
        },
        ...
        async assignAsset() {
            if (this.$refs.assetEdit.validate()) {
            this.isLoading = true
            await axios.post(
                    "/assign-asset/" + this.assetEdit.id, {
                        staff_id: this.assetEdit.staff_id,
                    }
                ).then(response => {
                    if (response.data.success) {
                        // Success logic
        ...
        unassignAssetmodal(item) {
            this.electronicUn = item
            this.dialogAssetUnassign = true
        },
        async unassignAsset() {
            if (this.$refs.unassign.validate()) {
            this.isLoading = true
            await axios.post(
                    "/unassign-electronic/" + this.electronicUn.id, {
                        condition: this.unassign.condition,
                        reason: this.unassign.reason,
                    }
                ).then(response => {
                    if (response.data.success) {
                        // Success logic
            ...
            this.dialogAssetUnassign = false
