...
<v-container fluid class="message">
    ...
    <v-dialog v-model="dialog" persistent max-width="600px">
        <template v-slot:activator="{ on, attrs }">
            <v-row class="header-row">
                <v-col cols="6">
                    <h2 class="mx-3 my-3">Sections</h2>
                </v-col>
        ...
        <v-card class="rounded-lg">
            ...
            <v-card-text class="my-5">
                <v-form @click.prevent="addSection">
                    <v-row>
                        <v-col cols="12">
                            <v-text-field type="text" label="Name" color="blue " class="rounded-lg" outlined dense required v-model="section.name"></v-text-field>
                        </v-col>
                        <!-- <v-col cols="12">
            ...
            <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn class="blue  white--text btn rounded-lg" text elevation="24" type="submit" @click="addSection">
                    Save
                </v-btn>
            </v-card-actions>
    ...
    <v-card class="mt-2 rounded-lg" elevation="24">
        <v-card-text>
            ...
            <v-data-table :headers="headers" :items="sections" :page.sync="page" :items-per-page="itemsPerPage" hide-default-footer :search="search" class="elevation-0 mx-8" @page-count="pageCount = $event">
                <template v-slot:[`item.actions`]="{ item }">
                    <v-icon small class="mr-2" @click="editSection(item)" color="blue " title="Edit">
                        mdi-pencil
                    </v-icon>
            ...
            <v-row class="pagination-row mx-8">
                <v-col cols="6">
                    <p class="items-range">
                       <strong>Showing {{ startIndex }} - {{ endIndex }} out of {{ sections.length }}</strong> 
                    </p>
                </v-col>
                <v-col cols="6" class="text-right">
    ...
    <v-dialog v-model="confirmDialogVisible" max-width="400px">
        <v-card class="rounded-xl" :elevation="24">
            ...
            <v-card-text class="text-h5 text--bold my-5 text-center " color="blue "> Are you sure you want to delete this <b class=""> {{ sectionToDelete.name }}?</b> </v-card-text>
            <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn color="blue " elevation="24" @click="confirmDialogVisible = false" class="rounded-lg btn white--text">Cancel</v-btn>
                <v-btn color="red" elevation="24" @click="deleteSection" class="rounded-lg btn white--text">OK</v-btn>
                <v-spacer></v-spacer>
            </v-card-actions>
        </v-card>
    </v-dialog>
        ...
        <v-card>
            ...
            <v-card-text class="my-5">
                <v-form>
                    <v-row>
                        <v-col cols="12">
                            <v-text-field type="text" class="rounded-lg" outlined dense v-model="sectionEdit.name" color="blue "></v-text-field>
                        </v-col>
                    </v-row>
                </v-form>
            </v-card-text>
            <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn class="blue  white--text btn rounded-lg" text elevation="24" type="submit" @click="updateSection">
                    Save
                </v-btn>
            </v-card-actions>
        </v-card>
...
export default {
    ...
    computed: {
        ...
        endIndex() {
            ...
            return end > this.sections.length ? this.sections.length : end;
        },
        customDateFormat() {
            return value => {
                if (!value) return '';
    ...
    methods: {
        ...
        addSection() {
            axios
                .post('/section-store', this.section)
                .then(response => {
        ...
        deleteSection() {
            axios.delete(`section-delete/${this.sectionToDelete.id}`)
                .then(response => {
                    // Remove the item from the data arraythis.dialogSection = true
                    const index = this.sections.indexOf(this.sectionToDelete);
                    if (index > -1) {
