...
<v-container fluid class="message ">
    ...
    <v-card elevation="24" class="rounded-lg my-3">
        <v-card-title class="light-blue lighten-5 my-5 blue--text ">
            ...
            <v-spacer></v-spacer>
            <h4 class="text-h5"> Assets Allocated to Section</h4>
            <v-spacer></v-spacer>
            ...
            <v-form @submit.prevent="fetchAssetData">
                <v-row>
                    ...
                    <v-col cols="12" sm="4" md="4" lg="4">
                        <v-autocomplete label="--Select Section--" item-text="name" class="rounded-lg" outlined dense item-value="name" color="blue lighten-1" v-model="sectionName" :items="sections"></v-autocomplete>
                    </v-col>
        ...
        <v-card-text v-if="assetsnotFound" class="text-center">
            <div class="">
                <v-alert text dense color="red" class="text-h5 mx-8" border="left" icon="mdi-delete-empty" dismissible>
                    No assets allocated to this section
                </v-alert>
            </div>
        </v-card-text>
    </v-card>
    ...
    <v-card class="rounded-lg" elevation="24" v-if="assetsFound">
        <v-card-text>
            ...
            <v-data-table :headers="headers" :items="assetsBySection" :page.sync="page" :items-per-page="itemsPerPage" hide-default-footer :search="search" class="elevation-0 " @page-count="pageCount = $event">
            ...
            <v-row class="pagination-row mx-8">
                <v-col cols sm="6" md="6" lg="6">
                    <p class="items-range blue--text text-darken-4">
                        <b>Showing {{ startIndex }} - {{ endIndex }} out of {{ assetsBySection.length }}</b>
                    </p>
                </v-col>
    ...
    <!--    
        ...
        <div v-if="assetsBySection.length === 0" class="text-center">
...
export default {
    ...
    computed: {
        ...
        endIndex() {
            ...
            return end > this.assetsBySection.length ? this.assetsBySection.length : end;
        },
    ...
    methods: {
        ...
        fetchAssetData() {
            this.loading = true;
            axios.get(`/assets-by-section/${this.sectionName}/${this.categoryName}`)
                .then(response => {
                    // Handle the successful response
                    this.assetsBySection = response.data.data;
                    ...
                    this.assetsFound = this.assetsBySection.length > 0;
                    this.assetsnotFound = this.assetsBySection.length === 0;
        ...
        <div style="text-align: center; padding: 10px; margin-top:10px; color: #42A5F5;">
          <div style="display: flex; justify-content: space-between; align-items: center;">
            ...
            <img src="${ttcl}" style="width: 230px; height: 180px;" />
          </div>
        </div>
        <div style="padding: 10px; font-size: 20px; font-family: 'Open Sans', sans-serif; margin-top: 5px;">
          <p style="text-align: center; margin: 0; ">Section:<b style="">${this.sectionName}</b> </p>
        </div>
      `;
        ...
        <thead>
          <tr style="background-color: #607D8B; color: white;">
            ...
            <th style="padding: 5px; text-align: center;">Electronic Name</th>
            <th style="padding: 5px; text-align: center;">Furniture Name</th>
        ...
        <tbody>`;
            ...
            this.assetsBySection.forEach((item, index) => {
        ...
        <div style="text-align: center; margin-top: 20px; font-size: 16px; font-family: 'Open Sans', sans-serif; style="padding: 20px;">
          <p>Phone(s): <span style="color: #42A5F5;"> <b>${this.Phone}</b>  </span>,       Computer(s): <span style="color: #42A5F5;"> <b>${this.Computer}</b> </span></p>
          <p>Television(s): <span style="color: #42A5F5;"> <b>${this.Television}</b> </span>,     Disk(s): <span style="color: #42A5F5;"> <b>${this.Disk}</b>  </span></p>
          <p>Chair(s): <span style="color: #42A5F5;"> <b>${this.Chair}</b> </span>,     Table(s):<span style="color: #42A5F5;"> <b>${this.Table}</b>  </span></p>
          <p>Locker(s):<span style="color: #42A5F5;"> <b>${this.Locker}</b> </span>,</p>
        </div>
      `;
        ...
        generatePDFReport() {
            ...
            const sectionNameText = `Office: ${this.sectionName}`;
            const sectionNameWidth = pdf.getStringUnitWidth(sectionNameText) * 12 / pdf.internal.scaleFactor;
            const centerPosition = (pageWidth - sectionNameWidth) / 2;
            pdf.text(sectionNameText, centerPosition, 40);

            // Calculate the position for the table
            const tableStartY = 50; // Adjust the Y position as needed

            // Add a table for the item details
            const tableData = this.assetsBySection.map(item => [
